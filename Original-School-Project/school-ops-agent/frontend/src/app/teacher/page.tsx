...
import { AlertTriangle, Bell, FileCheck, Send, Upload } from "lucide-react";
          ...
          {dash.blocked_students.length === 0 ? (
            ...
            <ul className="space-y-2">
              {dash.blocked_students.map((b, i) => (
                <li key={i} className="flex items-start gap-2 border border-blocked/30
                                       bg-blocked/5 rounded-md p-2">
                  <AlertTriangle size={16} className="text-blocked mt-0.5" />
                  <div>
                    <div className="machine text-xs text-muted">
                      student {b.student_id.slice(0, 8)}
                    </div>
                    <div className="text-sm text-ink">{b.note || "marked blocked"}</div>
                  </div>
                </li>
              ))}
            </ul>
          ...
          {dash.submissions_to_review.length === 0 ? (
            ...
            <ul className="space-y-2">
              {dash.submissions_to_review.map((s) => (
                <li key={s.id} className="border border-line rounded-md p-2">
                    ...
                    <div className="mt-2">
                      <textarea value={feedbackText}
                        ...
                        className="w-full px-2 py-1.5 rounded-md border border-line
                                   bg-surface text-sm mb-2" rows={2} />
                      <div className="flex gap-2">
                        <Button variant="ghost"
                          onClick={() => sendFeedback(s.id, "revision")}>
                          Request revision
                        </Button>
